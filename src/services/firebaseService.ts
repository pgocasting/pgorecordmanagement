...
export const pageVisibilityService = {
  async getPageVisibility() {
    try {
      const docs = await getItems<PageVisibility>('pageVisibility');
      const map: Record<string, boolean> = {};
      for (const d of docs) {
        map[d.name] = !!d.enabled;
      }
      return map;
    } catch (error) {
      console.error('Error getting page visibility:', error);
      return {} as Record<string, boolean>;
    }
  },

  async setPageVisibility(visibility: Record<string, boolean>) {
    try {
      const existingDocs = await getItems<PageVisibility>('pageVisibility');
      for (const existingDoc of existingDocs) {
        await deleteItem('pageVisibility', existingDoc.id);
      }
      for (const [name, enabled] of Object.entries(visibility)) {
        await addItem<PageVisibility>('pageVisibility', { name, enabled } as any);
      }
      return true;
    } catch (error) {
      console.error('Error setting page visibility:', error);
      return false;
    }
  },

  async updatePageVisibility(pageName: string, enabled: boolean) {
    try {
      const existingDocs = await getItems<PageVisibility>('pageVisibility');
      const existing = existingDocs.find(d => d.name === pageName);
      if (existing) {
        await updateItem<PageVisibility>('pageVisibility', existing.id, { enabled } as any);
      } else {
        await addItem<PageVisibility>('pageVisibility', { name: pageName, enabled } as any);
      }
      return true;
    } catch (error) {
...
export const systemSettingsService = {
  async getAllowUserThemes(): Promise<boolean> {
    try {
      const docs = await getItems<SystemSetting>('systemSettings');
      const existing = docs.find(d => d.key === 'allowUserThemes');
      return existing ? !!existing.enabled : true;
    } catch (error) {
      console.error('Error getting allowUserThemes setting:', error);
      return true;
    }
  },

  async setAllowUserThemes(enabled: boolean): Promise<boolean> {
    try {
      const docs = await getItems<SystemSetting>('systemSettings');
      const existing = docs.find(d => d.key === 'allowUserThemes');

      if (existing) {
        await updateItem<SystemSetting>('systemSettings', existing.id, { enabled } as any);
      } else {
        await addItem<SystemSetting>('systemSettings', { key: 'allowUserThemes', enabled } as any);
      }
