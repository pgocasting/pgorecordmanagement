...
interface AuthContextType {
  ...
  recoverUsers: () => void; // Add manual recovery function
  getRecords: () => Record[];
...
const getStoredUsers = (): User[] => {
  ...
  if (inMemoryUsers.length > 1) {
    return inMemoryUsers;
  }
  
  // If only default admin user exists, try to load from localStorage backup
  try {
    const stored = localStorage.getItem('storedUsers');
    if (stored) {
      const parsedUsers = JSON.parse(stored);
      if (Array.isArray(parsedUsers) && parsedUsers.length > 0) {
        inMemoryUsers = parsedUsers;
        return parsedUsers;
      }
    }
  } catch (error) {
...
const saveStoredUsers = (users: User[]): void => {
  ...
  try {
    localStorage.setItem('storedUsers', JSON.stringify(ensuredUsers));
  } catch (error) {
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const isChezka = username === 'pgo-chezka' || name === 'chezka mitzie corpuz';
    const isMeng = username === 'pgo-meng' || name === 'merlyn de leon';

    const root = document.documentElement;

    if (!allowUserThemes) {
      root.classList.remove('chezka-theme');
      root.classList.remove('meng-theme');
      return;
    }

    root.classList.toggle('chezka-theme', isChezka);
    root.classList.toggle('meng-theme', isMeng);
  ...
  }, [user, allowUserThemes]);

  // Initialize on mount
  useEffect(() => {
    const initializeUsers = async () => {
      try {
        // First, try to recover users from localStorage backup
        const localStorageUsers = localStorage.getItem('storedUsers');
        if (localStorageUsers) {
          try {
            const parsedUsers = JSON.parse(localStorageUsers);
            if (Array.isArray(parsedUsers) && parsedUsers.length > 0) {
              console.log('Recovered users from localStorage backup:', parsedUsers.length, 'users');
        ...
        if (!querySnapshot.empty) {
          ...
          console.log('Loaded users from Firebase:', firebaseUsers.length, 'users');
        } else {
          // If no users in Firebase, use default users
          saveStoredUsers(DEFAULT_USERS);
          console.log('Using default users (no Firebase users found)');
        }
      } catch (error) {
      ...
      if (currentUser) {
        ...
        if (currentUser.username === 'admin') {
          if (currentUser.role !== 'admin' || currentUser.name !== 'Administrator') {
      ...
      try {
        ...
        localStorage.setItem('allowUserThemes', enabled ? 'true' : 'false');
      } catch {
        const stored = localStorage.getItem('allowUserThemes');
        if (stored === 'false') {
          setAllowUserThemes(false);
        }
      }
  ...
  useEffect(() => {
    const handleSystemSettingsUpdated = async () => {
      try {
        const enabled = await systemSettingsService.getAllowUserThemes();
        setAllowUserThemes(enabled);
        localStorage.setItem('allowUserThemes', enabled ? 'true' : 'false');
        ...
        if (stored === 'false') {
          ...
          setAllowUserThemes(true);
        }
      }
    };

    window.addEventListener('systemSettingsUpdated', handleSystemSettingsUpdated as EventListener);
    return () => window.removeEventListener('systemSettingsUpdated', handleSystemSettingsUpdated as EventListener);
  }, []);
  ...
  const addUser = async (username: string, password: string, name: string, role: 'admin' | 'user', avatar?: string, email?: string): Promise<void> => {
    try {
      ...
      const newUser: Omit<User, 'id'> & { createdAt: string; updatedAt: string } = {
  ...
  const updateUser = async (userId: string, updates: Partial<User>): Promise<void> => {
    try {
      ...
      const updatedUser = { ...allUsers[userIndex], ...sanitizedUpdates };
  ...
  const recoverUsers = () => {
    // Manual recovery function to restore users from localStorage
    const localStorageUsers = localStorage.getItem('storedUsers');
    if (localStorageUsers) {
      try {
        const parsedUsers = JSON.parse(localStorageUsers);
        if (Array.isArray(parsedUsers) && parsedUsers.length > 0) {
          console.log('Manual recovery: Restored users from localStorage:', parsedUsers.length, 'users');
          saveStoredUsers(parsedUsers);
          return;
        }
      } catch (parseError) {
        console.warn('Manual recovery: Failed to parse localStorage users:', parseError);
      }
    }
    
    // Fallback to default users
    console.log('Manual recovery: Using default users');
    saveStoredUsers(DEFAULT_USERS);
  };
