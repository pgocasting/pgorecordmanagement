...
import { travelOrderService } from '@/services/firestoreService';
...
export default function TravelOrderPage() {
  ...
  useEffect(() => {
    const loadTravelOrders = async () => {
      try {
        ...
        setSuccess('Error loading travel orders');
  ...
  const handleAddTravelOrder = async () => {
    ...
    try {
      ...
      await travelOrderService.addTravelOrder(newTravelOrder);
  ...
  const handleDeleteTravelOrder = (id: string) => {
    setTravelOrderToDelete(id);
    setDeleteConfirmOpen(true);
  };

  const confirmDeleteTravelOrder = async () => {
    if (!travelOrderToDelete) return;

    setIsLoading(true);
    try {
      await travelOrderService.deleteTravelOrder(travelOrderToDelete);
      const updatedTravelOrders = await travelOrderService.getTravelOrders();
      setTravelOrders(updatedTravelOrders as TravelOrder[]);
      setSuccess('Travel order deleted successfully');
      setTravelOrderToDelete(null);
      setDeleteConfirmOpen(false);
      setSuccessModalOpen(true);
    } catch (err) {
      console.error('Failed to delete travel order:', err);
      setSuccess(err instanceof Error ? err.message : 'Error deleting travel order');
      setSuccessModalOpen(true);
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <Sheet open={sidebarOpen} onOpenChange={setSidebarOpen}>
        ...
        <SheetContent side="left" className="w-64 p-0">
          <TravelOrderSidebar recordTypes={recordTypes} onNavigate={() => setSidebarOpen(false)} />
        </SheetContent>
      </Sheet>

      {/* Desktop Sidebar */}
      <div className="hidden md:block w-64 bg-white border-r border-gray-200 shadow-sm">
        <TravelOrderSidebar recordTypes={recordTypes} onNavigate={undefined} />
      </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="bg-white rounded-lg border border-gray-200 shadow-md overflow-hidden">
              ...
              <Table>
                ...
                <TableBody>
                  {travelOrders.filter(t => t.status === 'Pending').length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={10} className="text-center py-4 text-gray-500 text-xs wrap-break-word whitespace-normal">
                        No pending travel orders. Click "Add Travel Order" to create one.
                      </TableCell>
                    </TableRow>
                  ) : (
                    travelOrders.filter(t => t.status === 'Pending').map((item) => (
                      <TableRow key={item.id} className="hover:bg-gray-50">
                        ...
                        <TableCell className="py-1 px-1 text-center wrap-break-word whitespace-normal">
                          <ActionButtons
                            ...
                            onDelete={() => handleDeleteTravelOrder(item.id)}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="flex gap-2 justify-end pt-4">
            ...
            <Button
              ...
              onClick={confirmDeleteTravelOrder}
              disabled={isLoading}
            >
      ...
      <Dialog open={timeOutConfirmOpen} onOpenChange={setTimeOutConfirmOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Enter the date/time out and any remarks for this travel order record.
            </DialogDescription>
          </DialogHeader>
...
function TravelOrderSidebar({ recordTypes, onNavigate }: any) {
  const { user } = useAuth();
  const navigate = useNavigate();
