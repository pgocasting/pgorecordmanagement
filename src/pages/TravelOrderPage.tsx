...
export default function TravelOrderPage() {
  ...
  const [travelOrderToReturn, setTravelOrderToReturn] = useState<string | null>(null);
  ...
  const filteredTravelOrders = travelOrders
    .filter((order) => {
      if ((order.status || 'Pending') !== activeTab) return false;

      const term = searchTerm.trim().toLowerCase();
      if (!term) return true;

      const designationAcronym = getDesignationAcronym(order.designation || '');
      ...
      const searchableFields = [
        ...
        designationAcronym,
        order.purpose,
        order.placeOfAssignment,
        order.remarks,
        order.timeOutRemarks,
      ];
  ...
  const generateTrackingId = (): string => {
    ...
    const todaysCount = travelOrders.filter(r => r.dateTimeIn && getPhilippinesDateKey(r.dateTimeIn) === todayKey).length;
  ...
  const handleEditTravelOrder = (id: string) => {
    ...
    if (travelOrder) {
      setFormData({
        trackingId: travelOrder.trackingId,
  ...
  const handleReturnTravelOrder = (id: string) => {
    setTravelOrderToReturn(id);
    setReturnData({ remarks: '' });
    setReturnConfirmOpen(true);
  };

  const confirmReturnTravelOrder = async () => {
    if (!travelOrderToReturn) return;
    ...
    setIsLoading(true);
    try {
      const travelOrder = travelOrders.find(t => t.id === travelOrderToReturn);
      ...
      await travelOrderService.updateTravelOrder(travelOrderToReturn, {
        status: 'Pending',
        remarks: newRemarks,
      ...
      const updated = travelOrders.map(t => t.id === travelOrderToReturn ? {
        ...t,
        status: 'Pending',
        remarks: newRemarks,
        remarksHistory: updatedRemarksHistory,
      ...
      setTravelOrders(updated);
      setSuccess('Travel order returned successfully');
      setTravelOrderToReturn(null);
      setReturnData({ remarks: '' });
      setReturnConfirmOpen(false);
      setSuccessModalOpen(true);
    } catch (err) {
      console.error('Failed to return travel order:', err);
      setSuccess(err instanceof Error ? err.message : 'Error returning travel order');
      setSuccessModalOpen(true);
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="flex h-screen bg-background">
        ...
        <div className="bg-white border-b shadow-sm pl-14 pr-4 sm:px-6 py-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold bg-gradient-to-r from-sky-600 to-sky-500 bg-clip-text text-transparent">Travel Order Records</h1>
                <p className="text-sm text-gray-500">Manage travel orders and assignments</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              {user?.name && (
                <div className="flex items-center gap-2.5 px-4 py-2.5 bg-gray-50 rounded-xl border border-gray-200">
                  <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-sky-500 to-sky-600 flex items-center justify-center shrink-0 shadow-sm">
        ...
        <div className="flex-1 overflow-hidden p-4 sm:p-6 bg-gray-50 flex flex-col min-h-0">
          <div className="border-0 shadow-md rounded-lg bg-white overflow-hidden flex flex-col flex-1 min-h-0">
            ...
            <div className="px-4 sm:px-6 py-4 border-b bg-gradient-to-r from-gray-50 to-white flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-lg font-semibold text-gray-900">Travel Orders</h2>
                <p className="text-sm text-gray-500 mt-1">Manage and view all travel order records</p>
              </div>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div className="relative w-full sm:w-64">
                  ...
                  <Input
                    ...
                    className="pl-10 h-10 border-gray-300 focus:border-sky-500 focus:ring-sky-500"
                  />
                </div>
                <Dialog open={isDialogOpen} onOpenChange={handleDialogOpenChange}>
                  <DialogTrigger asChild>
                    <Button
                      className="gap-2 bg-sky-600 hover:bg-sky-700 w-full sm:w-auto h-10"
                      onClick={() => {
                        setEditingId(null);
                        setFormData({
      ...
      <Dialog open={returnConfirmOpen} onOpenChange={setReturnConfirmOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Return Travel Order</DialogTitle>
            <DialogDescription>
              Provide a reason for returning this travel order. The status will be set back to "Pending".
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button className="bg-purple-600 hover:bg-purple-700" onClick={confirmReturnTravelOrder}>
              Return
            </Button>
          </DialogFooter>
        </DialogContent>
            ...
            <div className="flex gap-2 mt-4 border-b border-gray-200 px-4 sm:px-6 shrink-0">
              ...
              >
                Pending ({travelOrders.filter(r => (r.status || 'Pending') === 'Pending').length})
              </button>
              <button
              ...
              >
                Completed ({travelOrders.filter(r => r.status === 'Completed').length})
              </button>
              <button
              ...
              >
                Rejected ({travelOrders.filter(r => r.status === 'Rejected').length})
              </button>
            </div>

            {/* Table */}
            <div className="bg-card rounded-lg border shadowsm overflow-hidden flex-1 flex flex-col min-h-0">
                          ...
                          <div className="flex flex-col items-center gap-1">
                            ...
                            {(item.status === 'Completed' || item.status === 'Rejected') && (
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => handleReturnTravelOrder(item.id)}
