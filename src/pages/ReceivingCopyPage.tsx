...
export default function ReceivingCopyPage() {
  ...
  const filteredRecords = allRecords
    .filter((record) => selectedCategories.has(record.recordType || ''))
    ...
    <div className="flex h-screen bg-gray-100">
        ...
        <div className="bg-gradient-to-br from-gray-50 to-white border-b pl-14 pr-4 sm:px-6 py-3">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Button
                onClick={handlePrint}
                className="gap-2 bg-violet-600 hover:bg-violet-700 h-9"
              >
        ...
        <div className="flex-1 overflow-hidden p-6 flex flex-col">
          ...
          <div className="mb-4 print-hide">
            <div className="relative max-w-md">
              ...
              <Input
                placeholder="Search (acronym, tracking ID, name, category, office, particulars, remarks...)"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="pl-10 h-10 border-violet-200 focus:border-violet-400 focus:ring-violet-400"
              />
            </div>
          </div>

          {/* Category Filter */}
          <Card className="mb-4 print-hide shrink-0 border-0 shadow-md">
            <CardHeader className="pb-3 bg-gradient-to-br from-gray-50 to-white">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  className="text-xs text-violet-600 hover:text-violet-700 font-medium p-0 h-auto"
                >
          ...
          <Card className="overflow-hidden flex flex-col flex-1 min-h-0 border-0 shadow-md">
            {isLoading ? (
                ...
                <table className="w-full h-full border-collapse table-fixed">
                  ...
                  <tbody>
                    {paginatedRecords.map((record, index) => (
                      <tr key={record.id} className={`border-b border-gray-200 ${selectedRecords.has(record.id) ? 'bg-violet-50 print-selected' : index % 2 === 0 ? 'bg-white hover:bg-gray-50' : 'bg-gray-50/50 hover:bg-gray-100'}`}>
                        ...
                        <td className="border border-gray-300 px-2 py-2 text-sm text-gray-900 wrap-break-word">
                          {getFullName(record)}
                        </td>
                        <td className="border border-gray-300 px-2 py-2 text-sm text-gray-900 wrap-break-word">
                        ...
                        <td className="border border-gray-300 px-2 py-2 text-sm text-gray-900 wrap-break-word print-hide">
            ...
            {!isLoading && filteredRecords.length > 0 && (
              <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-violet-50 to-purple-50 border-t-2 border-violet-200 print-hide shrink-0">
                <div className="text-sm text-gray-700">
                  Showing <span className="font-medium text-violet-600">{startIndex + 1}</span> to{' '}
                  <span className="font-medium text-violet-600">{Math.min(endIndex, filteredRecords.length)}</span> of{' '}
                  <span className="font-medium text-violet-600">{filteredRecords.length}</span> records
                </div>
                      ...
                      if (
                        ...
                        return (
                          <Button
                            ...
                            size="sm"
                            className={currentPage === page ? 'bg-violet-600 hover:bg-violet-700 text-white' : 'border-violet-200 hover:bg-violet-50'}
                          >
                            {page}
                          </Button>
                        );
      ...
      <style>{`
        ...
        @media print {
          ...
          th:first-child, 
          td:first-child,
          th:nth-child(10), /* Remarks header */
          td:nth-child(10) { /* Remarks data cells */
            display: none !important;
          }
          /* Add border to amount column in print */
          th:nth-child(8), /* Amount header */
          td:nth-child(8) { /* Amount data cells */
            border: 1px solid #000 !important;
          }
