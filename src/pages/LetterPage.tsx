...
import { Plus, Menu, LogOut, Search, Mail } from 'lucide-react';
...
export default function LetterPage() {
  ...
  const [letterToReturn, setLetterToReturn] = useState<string | null>(null);
  ...
  const filteredLetters = letters
    .filter((letter) => {
      if ((letter.status || 'Pending') !== activeTab) return false;

      const term = searchTerm.trim().toLowerCase();
      if (!term) return true;

      const designationAcronym = getDesignationAcronym(letter.designationOffice || '');

      const searchableFields = [
        letter.trackingId,
        letter.fullName,
        letter.receivedBy,
        letter.designationOffice,
        designationAcronym,
        letter.particulars,
        letter.linkAttachments,
        letter.remarks,
        letter.timeOutRemarks,
      ];
  ...
  const generateTrackingId = (): string => {
    ...
    const todaysCount = letters.filter(r => r.dateTimeIn && getPhilippinesDateKey(r.dateTimeIn) === todayKey).length;
    const sequenceNumber = String(todaysCount + 1).padStart(3, '0');
    ...
    try {
      ...
      if (editingId) {
        ...
        const updateData = {
          ...
          linkAttachments: formData.linkAttachments,
          remarks: formData.remarks,
        ...
        const nextTrackingId = formData.trackingId || generateTrackingId();
        const { trackingId: _trackingId, ...restFormData } = formData;
        const newLetter = {
  ...
  const handleEditLetter = (letter: Letter) => {
    setFormData({
      ...
      linkAttachments: letter.linkAttachments || '',
  ...
  const handleTimeOut = (letterId: string) => {
    ...
    const dateTimeLocal = getCurrentDateTime();
  ...
  const handleReturnLetter = (id: string) => {
    setLetterToReturn(id);
    setReturnData({ remarks: '' });
    setReturnConfirmOpen(true);
  };

  const confirmReturnLetter = async () => {
    ...
    setIsLoading(true);
    try {
      const letter = letters.find(l => l.id === letterToReturn);
      if (!letter) return;
      const now = new Date().toISOString();
      const currentUser = user?.name || 'Unknown';
      ...
      await letterService.updateLetter(letterToReturn, {
        status: 'Pending',
        remarks: newRemarks,
        remarksHistory: updatedRemarksHistory,
        dateTimeOut: '',
      ...
      const updated = letters.map(l => l.id === letterToReturn ? {
        ...l,
        status: 'Pending',
        remarks: newRemarks,
        remarksHistory: updatedRemarksHistory,
      ...
      setLetters(updated);
      setSuccess('Letter returned successfully');
      setLetterToReturn(null);
      setReturnData({ remarks: '' });
      setReturnConfirmOpen(false);
      setSuccessModalOpen(true);
    } catch (err) {
      console.error('Failed to return letter:', err);
      setSuccess('Error returning letter');
      setSuccessModalOpen(true);
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="flex h-screen bg-background">
        ...
        <div className="bg-white border-b shadow-sm pl-14 pr-4 sm:px-6 py-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-blue-500 bg-clip-text text-transparent">Letter Records</h1>
                <p className="text-sm text-gray-500">Manage incoming and outgoing letters</p>
              </div>
            </div>
          ...
          <div className="border-0 shadow-md rounded-lg bg-white overflow-hidden h-full flex flex-col">
            ...
            <div className="px-4 sm:px-6 py-4 border-b bg-gradient-to-r from-gray-50 to-white flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Manage and view all letter records</p>
              </div>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <Dialog open={isDialogOpen} onOpenChange={handleDialogOpenChange}>
                  <DialogTrigger asChild>
                    <Button
                      className="gap-2 bg-blue-600 hover:bg-blue-700 w-full sm:w-auto h-10"
                      onClick={() => {
                        setEditingId(null);
                        setFormData({
                  ...
                  <div className="grid gap-4 py-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => setFormData({ ...formData, linkAttachments: e.target.value })}
                      />
                    </div>
                  </div>
                  <Button
      ...
      <Dialog open={returnConfirmOpen} onOpenChange={setReturnConfirmOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Return Letter</DialogTitle>
            <DialogDescription>
              Provide a reason for returning this letter. The status will be set back to "Pending".
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button className="bg-purple-600 hover:bg-purple-700" onClick={confirmReturnLetter}>
              Return
            </Button>
          </DialogFooter>
        </DialogContent>
            ...
            <div className="flex gap-2 mt-4 border-b border-gray-200 px-4 sm:px-6 shrink-0">
              ...
              >
                Pending ({letters.filter(r => (r.status || 'Pending') === 'Pending').length})
              </button>
              <button
              ...
              >
                Completed ({letters.filter(r => r.status === 'Completed').length})
              </button>
              <button
              ...
              >
                Rejected ({letters.filter(r => r.status === 'Rejected').length})
              </button>
            </div>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-muted/50 sticky top-0 z-10">
                        ...
                        <TableHead className="font-semibold py-3 px-4 text-center text-xs whitespace-normal wrap-break-word max-w-[240px]">Particulars</TableHead>
                  ...
                  {filteredLetters.length === 0 ? (
                    ...
                    filteredLetters.map((item) => (
                      <TableRow key={item.id} className="hover:bg-muted/50 transition-colors">
                        ...
                        <TableCell className="text-sm py-3 px-4 text-center whitespace-normal wrap-break-word max-w-[240px]">{item.particulars}</TableCell>
                        <TableCell className="text-sm py-3 px-4 text-center">
                          <Badge 
                            variant={
                            ...
                            <div className="space-y-1 relative">
                              ...
                              <div className="text-black">
                                {item.remarksHistory?.length > 0
                                  ? item.remarksHistory[item.remarksHistory.length - 1].remarks
                                  : item.remarks}
                              </div>
                              {item.remarksHistory?.length > 0 && (
                          ...
                          <div className="flex flex-col items-center gap-1">
                            ...
                            {(item.status === 'Completed' || item.status === 'Rejected') && (
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => handleReturnLetter(item.id)}
                ...
                <Card className="border">
                  <CardContent className="p-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-600">Link/Attachment</p>
                        {selectedLetter.linkAttachments ? (
                          <a
                            href={
                              selectedLetter.linkAttachments.startsWith('http://') ||
                              selectedLetter.linkAttachments.startsWith('https://')
                                ? selectedLetter.linkAttachments
                                : `https://${selectedLetter.linkAttachments}`
                            }
                            target="_blank"
                            rel="noreferrer"
                            className="text-sm text-blue-600 hover:underline mt-1 block wrap-break-word"
                          >
                            {selectedLetter.linkAttachments}
                          </a>
                        ) : (
